...
function ToggleButton({ active, onClick, children }: { active: boolean; onClick: () => void; children: React.ReactNode }) {
  return (
    <button onClick={onClick} style={{ border: active ? '1px solid #102033' : '1px solid rgba(16,32,51,0.12)', background: active ? '#102033' : 'rgba(255,255,255,0.92)', color: active ? '#ffffff' : '#102033', borderRadius: 999, padding: '10px 14px', fontWeight: 800, fontSize: 13, cursor: 'pointer' }}>
      {children}
    </button>
  )
}
...
function DistrictButton({ active, onClick, children }: { active: boolean; onClick: () => void; children: React.ReactNode }) {
  return (
    <button onClick={onClick} style={{ border: active ? '1px solid #2f8c63' : '1px solid rgba(16,32,51,0.12)', background: active ? 'rgba(47,140,99,0.12)' : '#ffffff', color: '#102033', borderRadius: 14, padding: '12px 14px', fontWeight: 800, fontSize: 13, cursor: 'pointer', textAlign: 'left' }}>
      {children}
    </button>
  )
}
...
function MapDemo({ district, stage }: { district: DistrictPreset; stage: Stage }) {
  ...
  return (
    <div style={{ marginTop: 16, height: 380, borderRadius: 24, position: 'relative', overflow: 'hidden', background: 'linear-gradient(180deg, #eff6ff 0%, #f0fdf4 46%, #fef3c7 100%)', border: '1px solid rgba(16,32,51,0.08)' }}>
      <div style={{ position: 'absolute', inset: 0, backgroundImage: 'linear-gradient(rgba(16,32,51,0.04) 1px, transparent 1px), linear-gradient(90deg, rgba(16,32,51,0.04) 1px, transparent 1px)', backgroundSize: '52px 52px' }} />
      <svg style={{ position: 'absolute', inset: 0, width: '100%', height: '100%' }}>
        {district.growers.map((grower, idx) => (
          <line key={grower.id} x1={`${grower.x}%`} y1={`${grower.y}%`} x2={`${district.store.x}%`} y2={`${district.store.y}%`} stroke={routesStrong ? '#2f8c63' : '#7aa5c7'} strokeWidth={routesStrong ? 4 : 3} strokeDasharray="10 8" opacity={routesVisible ? 1 : 0.1} style={{ transition: `opacity 400ms ease ${idx * 180}ms, stroke 400ms ease, stroke-width 400ms ease` }} />
        ))}
      </svg>
      {district.growers.map((grower) => (
        <div key={grower.id} style={{ position: 'absolute', left: `${grower.x}%`, top: `${grower.y}%`, transform: 'translate(-50%, -50%)' }}>
          <div style={{ width: 16, height: 16, borderRadius: 999, background: colorForFruit(grower.fruit), boxShadow: stage === 'completed' ? `0 0 0 10px ${colorForFruit(grower.fruit)}33` : `0 0 0 8px ${colorForFruit(grower.fruit)}22` }} />
          <div style={{ marginTop: 8, padding: '6px 10px', borderRadius: 999, background: 'rgba(255,255,255,0.95)', fontSize: 11, fontWeight: 800, whiteSpace: 'nowrap', boxShadow: '0 8px 18px rgba(16,32,51,0.08)' }}>{grower.name}</div>
        </div>
      ))}
      <div style={{ position: 'absolute', left: `${district.store.x}%`, top: `${district.store.y}%`, transform: 'translate(-50%, -50%)' }}>
        <div style={{ width: 20, height: 20, borderRadius: 999, background: '#102033', boxShadow: stage === 'store-execution' || stage === 'completed' ? '0 0 0 14px rgba(16,32,51,0.18)' : '0 0 0 8px rgba(16,32,51,0.10)' }} />
        <div style={{ marginTop: 8, padding: '6px 10px', borderRadius: 999, background: 'rgba(255,255,255,0.95)', fontSize: 11, fontWeight: 900, whiteSpace: 'nowrap', boxShadow: '0 8px 18px rgba(16,32,51,0.08)' }}>{district.store.name}</div>
      </div>
    </div>
  )
}
...
export default function DistrictManagerMode({ listings }: { listings: Listing[] }) {
  ...
  const stats = useMemo(() => {
    ...
    return { stores: '12', growers: growers.toString(), supply: `${supply.toLocaleString()} units` }
  }, [listings])
  ...
  async function runExecution() {
    ...
    setSummary(`Scanning ${district.name} for shortage pressure and nearby local inventory.`)
    ...
    setEta('22 min')
    setSummary(`Matching ${district.growers.length} growers to ${district.store.name} for ${district.store.shortageFruit}.`)
    await new Promise((r) => setTimeout(r, 1100))

    const secured = district.growers.reduce((sum, g) => sum + g.units, 0)
    ...
    setStage('completed')
    setCoverage(100)
    setSummary(`Local fill executed. ${secured} units secured for ${district.store.name}.`)
    setIsRunning(false)
  }
  ...
  const buttonLabel = stage === 'idle' ? 'Execute Local Fill' : stage === 'market-scan' ? 'Scanning Market...' : stage === 'supplier-match' ? 'Matching Suppliers...' : stage === 'store-execution' ? 'Executing Routes...' : 'Fill Executed'

  const roleSubtitle = role === 'district-manager' ? 'Store-by-store fill visibility for district execution.' : role === 'regional-vp' ? 'Regional operating signal across stores, growers, and timing.' : 'National retail framing for scalable hyperlocal supply deployment.'
  ...
  return (
    <div style={{ minHeight: '100vh', fontFamily: 'Inter, Arial, sans-serif', background: 'linear-gradient(180deg, #f8fbff 0%, #edf3f8 100%)', color: '#102033', padding: 28 }}>
      <div style={{ maxWidth: 1360, margin: '0 auto' }}>
        <section style={{ borderRadius: 34, overflow: 'hidden', padding: 38, display: 'grid', gridTemplateColumns: '1.2fr 0.8fr', gap: 24, background: 'linear-gradient(135deg, #1f6d5b 0%, #2e7d6b 18%, #497fa6 48%, #c16b2f 82%, #d5923a 100%)', boxShadow: '0 24px 60px rgba(16,32,51,0.18)' }}>
          <div>
            <Pill dark>PLUCK · DISTRICT EXECUTION</Pill>
            <div style={{ marginTop: 18, color: '#ffffff', fontSize: 62, lineHeight: 0.94, fontWeight: 900, letterSpacing: '-0.05em', maxWidth: 780 }}>Hyperlocal supply intelligence for food retail.</div>
            <div style={{ marginTop: 18, color: 'rgba(255,255,255,0.95)', fontSize: 21, lineHeight: 1.35, maxWidth: 760 }}>See shortages, match nearby grower supply, and execute district-level fill opportunities across a national operating model.</div>
          ...
          <div style={{ background: 'rgba(255,255,255,0.92)', border: '1px solid rgba(16,32,51,0.08)', borderRadius: 22, boxShadow: '0 18px 44px rgba(16,32,51,0.10)', padding: 24, alignSelf: 'end' }}>
            ...
            <button onClick={runExecution} disabled={isRunning} style={{ marginTop: 18, border: 0, borderRadius: 16, padding: '14px 18px', background: isRunning ? '#6b7280' : '#102033', color: '#ffffff', fontWeight: 800, fontSize: 16, cursor: isRunning ? 'default' : 'pointer' }}>{buttonLabel}</button>
            <button onClick={runAutoPlay} disabled={isRunning || autoPlay} style={{ marginTop: 10, border: 0, borderRadius: 14, padding: '12px 16px', background: '#2f8c63', color: '#ffffff', fontWeight: 800, fontSize: 14, cursor: isRunning || autoPlay ? 'default' : 'pointer' }}>{autoPlay ? 'Running Executive Demo...' : 'Run Executive Demo'}</button>
          </div>
        </section>
        ...
        <section style={{ display: 'grid', gridTemplateColumns: '1.15fr 0.85fr', gap: 22, marginTop: 22 }}>
          ...
          <div style={{ display: 'grid', gap: 18 }}>
            ...
            <div style={{ background: 'rgba(255,255,255,0.92)', border: '1px solid rgba(16,32,51,0.08)', borderRadius: 22, boxShadow: '0 18px 44px rgba(16,32,51,0.08)', padding: 22 }}>
              ...
              <div style={{ marginTop: 10, fontSize: 28, fontWeight: 900, lineHeight: 1.04 }}>{stage === 'idle' ? 'Ready to execute' : stage === 'market-scan' ? 'Scanning market' : stage === 'supplier-match' ? 'Matching supply' : stage === 'store-execution' ? 'Executing routes' : 'Execution complete'}</div>
