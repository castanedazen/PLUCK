...
BACKUP_DIR="$ROOT/.pluck-backups/master-district-fix-$(date +%Y%m%d-%H%M%S)"

mkdir -p "$BACKUP_DIR"
cp "$APP" "$BACKUP_DIR/App.tsx.bak"
cp "$DISTRICT" "$BACKUP_DIR/DistrictManagerMode.tsx.bak" 2>/dev/null || true
...
if "goTo('/district')" not in text and "navigate('/district')" not in text:
    targets = [
        "<button className=\"ghost\" onClick={() => goTo('/map')}>Explore nearby map</button>",
        "<button className=\"ghost\" onClick={() => navigate('/map')}>Open map</button>",
        "<button className=\"ghost\" onClick={() => goTo('/map')}>Open map</button>",
        "<button className=\"ghost\" onClick={() => navigate('/map')}>Explore nearby map</button>",
    ]
    inserted = False
