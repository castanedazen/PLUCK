...
function getListingStage(listing: Listing) {
  if (listing.isSold || listing.status === 'sold') return 'sold'
  if (listing.reservedByUserId) return 'reserved'
  return 'available'
}

function getListingStageLabel(listing: Listing) {
  const stage = getListingStage(listing)
  if (stage === 'sold') return 'Sold'
  if (stage === 'reserved') return 'Reserved'
  return listing.availabilityLabel || 'Available now'
}

function statusToneClass(listing: Listing) {
  const stage = getListingStage(listing)
  if (stage === 'sold') return 'sold'
  if (stage === 'reserved') return 'reserved'
  return 'available'
...
function RatingStars({ value }: { value: number }) {
  ...
  return (
    <span className="rating-stars" aria-label={`${value.toFixed(1)} out of 5 stars`}>
      {'★★★★★'.slice(0, rounded)}
      <span className="rating-stars-dim">{'★★★★★'.slice(rounded)}</span>
    </span>
  )
}

function FruitHeroRibbon({ listings }: { listings: Listing[] }) {
  const heroItems = listings.slice(0, 4)

  return (
    <section className="fruit-hero-ribbon">
      <div className="fruit-hero-copy">
        <p className="eyebrow">Fresh from nearby trees</p>
        <h2>Smoother, brighter, more beautiful neighborhood produce discovery.</h2>
        <p>
          Lush imagery, clearer trust signals, real map browsing, pickup state clarity, and reviews now make Pluck feel like a premium local marketplace.
        </p>
      </div>

      <div className="fruit-hero-mosaic">
        {heroItems.map((item, index) => (
          <article className={`fruit-hero-tile tile-${index + 1}`} key={item.id}>
            <img src={item.image} alt={item.title} />
            <div className="fruit-hero-overlay">
              <span className={`trust-pill ${statusToneClass(item)}`}>{getListingStageLabel(item)}</span>
...
function normalizeListing(listing: Listing): Listing {
  ...
  return {
    ...
    isSold: listing.isSold ?? listing.status === 'sold',
    pickupCompletedAt: listing.pickupCompletedAt || null,
    reservedByUserId: listing.reservedByUserId || '',
    reservedByName: listing.reservedByName || '',
    reservedPickupWindow: listing.reservedPickupWindow || '',
  }
}
...
function NationwideGeoMap({
  ...
  return <LeafletMapView listings={listings} onOpenListing={onOpenListing} />
}
...
function ListingDetailRoute({
  ...
  currentUserProfile: SellerProfile
  ...
  onCompletePickup: (listingId: string) => Promise<void>
}) {
  const navigate = useNavigate()
  const { id } = useParams()
  ...
  const stage = getListingStage(listing)
  const canReserve = stage === 'available' && listing.inventory > 0
  const canComplete = !listing.isSold && Boolean(listing.reservedByUserId) && listing.sellerId === currentUserProfile.id
  const canReview =
    Boolean(authUser) &&
    authUser?.role === 'buyer' &&
    Boolean(listing.isSold) &&
    listing.reservedByUserId === currentUserProfile.id &&
    !reviews.some((review) => review.buyerId === currentUserProfile.id)

  async function handleSubmitReview(e: FormEvent) {
    e.preventDefault()
    if (!canReview || !authUser) return
    setReviewBusy(true)
    setReviewError('')
    try {
  ...
  return (
    <section className="stack">
      <div className="listing-detail-hero polished-detail-hero">
        ...
        <div className="listing-detail-copy">
          ...
          <div className="listing-top-badges">
            ...
            <span className={`trust-pill ${statusToneClass(listing)}`}>{getListingStageLabel(listing)}</span>
            {hasGeo(listing) ? <span className="trust-pill verified">Map ready</span> : null}
          </div>

          <div className="detail-rating-row">
            <RatingStars value={listing.sellerRating || 0} />
            <span>{sellerLabel(listing)}</span>
          </div>

          <p className="desc">{listing.description}</p>
          <p className="meta">
            {prettyLocation(listing)} • {listing.inventory} left
            {listing.reservedByName ? ` • Reserved by ${listing.reservedByName}` : ''}
          </p>
          ...
          <div className="pill-row">
            ...
            {listing.reservedPickupWindow ? <span className="pill accent-pill">Reserved: {listing.reservedPickupWindow}</span> : null}
          </div>

          <ActionGrid columns={2}>
            <button className="primary fill-btn" onClick={() => onOpenReserve(listing)} disabled={!canReserve}>
              {stage === 'sold' ? 'Sold out' : stage === 'reserved' ? 'Reserved' : 'Reserve pickup'}
            </button>
            ...
            {listing.sellerId === currentUserProfile.id ? (
              <button className="ghost fill-btn" onClick={() => onEdit(listing.id)}>
                Edit listing
              </button>
            ) : null}
            {canComplete ? (
              <button className="primary fill-btn success-btn" onClick={() => onCompletePickup(listing.id)}>
      ...
      <section className="reviews-panel polished-panel">
        <div className="section-heading compact-heading no-top-gap">
          <div>
            <p className="eyebrow">Trust layer</p>
            <h2>Reviews & pickup confidence</h2>
          </div>
          <div className="review-summary-chip">
            <RatingStars value={listing.sellerRating || 0} />
            <strong>{(listing.sellerRating || 0).toFixed(1)}</strong>
            <span>{reviews.length} reviews on this listing</span>
          </div>
        </div>

        {stage === 'reserved' && listing.reservedByName ? (
          <div className="inline-banner reserved-banner">
            Reserved by {listing.reservedByName} for {listing.reservedPickupWindow || 'pickup coordination'}.
          </div>
        ) : null}

        {stage === 'sold' ? (
          <div className="inline-banner sold-banner">
            This harvest has been marked sold{listing.pickupCompletedAt ? ` on ${formatShortDate(listing.pickupCompletedAt)}` : ''}.
          </div>
        ) : null}
          ...
          <form className="review-form" onSubmit={handleSubmitReview}>
            <div className="review-form-head">
              <div>
                ...
                <p>Let future buyers know how the pickup went.</p>
              </div>
            ...
            <textarea
              ...
              placeholder="Describe the pickup quality, communication, and fruit freshness."
              rows={4}
            />
            {reviewError ? <p className="error-text">{reviewError}</p> : null}
            <div className="review-form-actions">
              <button className="primary" type="submit" disabled={reviewBusy}>
              ...
              <article className="review-card" key={review.id}>
                <div className="review-card-head">
                  <div>
                    ...
                    <div className="detail-rating-row compact">
                      <RatingStars value={review.rating} />
                      <span>{review.rating.toFixed(1)}</span>
                    </div>
                  </div>
                  <span className="review-date">{formatShortDate(review.createdAt)}</span>
                </div>
                <p>{review.comment || 'Smooth pickup and fresh fruit.'}</p>
              </article>
            ))
          ) : (
            <div className="empty-panel">No reviews yet. The first completed pickup can set the tone here.</div>
          )}
        </div>
      </section>
    </section>
...
function AppLayout({
  ...
  async function handleCompletePickup(listingId: string) {
    const updated = normalizeListing(await completePickup({ listingId }))
    setListings((current) => current.map((item) => (item.id === listingId ? updated : item)))
  }
  ...
  return (
    <div className="app-shell">
      <header className="topbar">
        <div>
          <p className="eyebrow">Pluck final marketplace build</p>
          <h1>Fresh fruit from neighbors</h1>
          <p className="subtle-copy">
            A polished local marketplace for backyard harvests with map discovery, ratings, pickup lifecycle clarity, and smoother visual rhythm.
          </p>
        </div>
      ...
      <main className="content">
        <Routes>
          ...
          <Route
            ...
            element={
              <>
                <section className="hero-card lush-hero-card">
                  <div>
                ...
                <FruitHeroRibbon listings={filtered.length ? filtered : listings} />
                    ...
                    return (
                      <article className="card premium-card listing-card-v2" key={item.id}>
                        ...
                        <div className="card-body">
                          ...
                          <div className="detail-rating-row compact card-rating-row">
                            <RatingStars value={item.sellerRating || 0} />
                            <span>{(item.sellerRating || 0).toFixed(1)} seller rating</span>
                          </div>
          ...
          <Route
            ...
            element={
              <section className="map-panel">
                <div className="map-fallback premium-map real-map-shell">
                  ...
                  <div className="map-caption-row">
                    <span>Live OpenStreetMap layer</span>
                    <span>{filtered.filter(hasGeo).length} geo-ready listings</span>
                  </div>
                    ...
                    {myListings.map((item) => (
                      <div className="listing-row" key={item.id}>
                        ...
                        <div className="listing-row-copy">
                          ...
                          <span>{item.reservedPickupWindow || item.pickupWindows[0]}</span>
                          <div className="listing-badge-row">
                            {item.harvestLabel ? <span className="trust-pill harvest">{item.harvestLabel}</span> : null}
                            <span className={`trust-pill ${statusToneClass(item)}`}>{getListingStageLabel(item)}</span>
                            {item.reservedByName ? <span className="trust-pill reserved">Reserved by {item.reservedByName}</span> : null}
                          </div>
                        </div>
                        <div className="listing-row-actions">
                          ...
                          {!item.isSold && item.reservedByUserId ? (
                            <button className="primary compact-success" onClick={() => handleCompletePickup(item.id)}>
                              Mark sold
                            </button>
                          ) : null}
            ...
            element={
              <section className="stack">
                ...
                <div className="analytics-note">
                  ...
                  <p>Live map browsing, completed pickup states, richer reviews, and a more premium storefront presentation.</p>
                </div>
              </section>
            }
          />
