...
const routeMeta: { match: RegExp; eyebrow: string; title: string; subtitle: string }[] = [
  ...
  {
    match: /^\/district/,
    eyebrow: 'District mode',
    title: 'See shortages, supply, and fill routes at district scale.',
    subtitle: 'A retailer-facing view for matching neighborhood growers to store demand.',
  },
  {
    match: /^\/ops/,
    eyebrow: 'Ops console',
    title: 'Enter shortages, enter supply, and run live fill plans.',
    subtitle: 'A practical operations console for testing local supply execution.',
  },
  {
    match: /^\/map/,
    eyebrow: 'Field map',
  ...
  return (
    <div className="app-shell">
      ...
      <main id="listing-feed" className="content">
        ...
        {location.pathname === '/' ? (
          <div style={{ position: 'fixed', right: 20, bottom: 96, zIndex: 90, display: 'grid', gap: 10 }}>
            <button
              type="button"
              onClick={() => navigate('/district')}
              style={{ border: 0, borderRadius: 999, padding: '12px 16px', background: '#102033', color: '#fff', fontWeight: 800, boxShadow: '0 10px 24px rgba(16,32,51,0.22)', cursor: 'pointer' }}
            >
              District demo
            </button>
            <button
              ...
              style={{ border: 0, borderRadius: 999, padding: '12px 16px', background: '#2f8c63', color: '#fff', fontWeight: 800, boxShadow: '0 10px 24px rgba(47,140,99,0.22)', cursor: 'pointer' }}
            >
              Ops console
            </button>
          </div>
      ...
      <nav className="bottom-nav">
        ...
        <NavLink to="/district" className={({ isActive }) => (isActive ? 'nav-item active' : 'nav-item')}>
          District
        </NavLink>
