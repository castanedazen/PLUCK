...
APP="$CLIENT/App.tsx"
DISTRICT="$CLIENT/DistrictManagerMode.tsx"
STAMP="$(date +%Y%m%d-%H%M%S)"

if [ ! -f "$APP" ]; then
  echo "App.tsx not found at $APP"
  exit 1
fi

mkdir -p "$ROOT/.pluck-backups/$STAMP"
cp "$APP" "$ROOT/.pluck-backups/$STAMP/App.tsx.bak"
[ -f "$DISTRICT" ] && cp "$DISTRICT" "$ROOT/.pluck-backups/$STAMP/DistrictManagerMode.tsx.bak" || true

echo "Backups saved to $ROOT/.pluck-backups/$STAMP"
...
app = Path('/workspaces/PLUCK/client/src/App.tsx')
text = app.read_text()

if "import DistrictManagerMode from './DistrictManagerMode'" not in text:
    marker = "import CommunityBoard from './components/CommunityBoard'\n"
    if marker in text:
        text = text.replace(marker, marker + "import DistrictManagerMode from './DistrictManagerMode'\n", 1)

route_block = """
            <Route
              path=\"/district\"
              element={
                <DistrictManagerMode listings={listings} />
              }
            />
"""

if '<DistrictManagerMode listings={listings} />' not in text:
    insert_after = "            <Route path=\"/board\" element={<CommunityBoard posts={socialPosts} />} />\n"
    if insert_after in text:
        text = text.replace(insert_after, insert_after + route_block, 1)

app.write_text(text)
PY

echo "District page reinstalled."
echo "Now run: cd $ROOT/client && npm run dev -- --host 0.0.0.0"
