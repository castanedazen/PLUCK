...
BACKUP_DIR="$ROOT/.pluck-backups/build3-live-district-$(date +%Y%m%d-%H%M%S)"
...
type ExecuteResponse = {
  ...
  matches?: { grower: string; units: number; distance: string }[]
  fulfilled?: number
  remaining?: number
}
...
const districts: DistrictPreset[] = [
  {
    ...
    item: 'avocados',
    store: { id: '1423', name: 'Store 1423', city: 'North Hills', shortageFruit: 'Avocados', shortageQty: 180, x: 26, y: 50, lat: 34.24, lng: -118.48 },
    growers: [
      { id: 'g1', name: 'Cesar Grove', fruit: 'Avocados', units: 72, x: 12, y: 34, lat: 34.20, lng: -118.50 },
      { id: 'g2', name: 'Maria Orchard', fruit: 'Avocados', units: 48, x: 18, y: 72, lat: 34.10, lng: -118.40 },
      { id: 'g3', name: 'Valley Harvest', fruit: 'Avocados', units: 36, x: 40, y: 24, lat: 34.17, lng: -118.46 },
    ],
  },
  {
    id: 'la-core',
    ...
    item: 'tomatoes',
    store: { id: '1049', name: 'Store 1049', city: 'Reseda', shortageFruit: 'Tomatoes', shortageQty: 95, x: 52, y: 62, lat: 34.19, lng: -118.54 },
    growers: [
      { id: 'g4', name: 'South Central Growers', fruit: 'Tomatoes', units: 38, x: 28, y: 70, lat: 34.18, lng: -118.55 },
      { id: 'g5', name: 'Eastside Plot', fruit: 'Tomatoes', units: 29, x: 74, y: 38, lat: 34.16, lng: -118.57 },
      { id: 'g6', name: 'Mid City Harvest', fruit: 'Tomatoes', units: 24, x: 60, y: 22, lat: 34.20, lng: -118.52 },
    ],
  },
  {
    id: 'pasadena',
    ...
    item: 'lemons',
    store: { id: '1182', name: 'Store 1182', city: 'Pasadena', shortageFruit: 'Lemons', shortageQty: 70, x: 72, y: 48, lat: 34.20, lng: -118.45 },
    growers: [
      { id: 'g7', name: 'Foothill Citrus', fruit: 'Lemons', units: 28, x: 58, y: 24, lat: 34.19, lng: -118.44 },
      { id: 'g8', name: 'Arroyo Co-op', fruit: 'Lemons', units: 18, x: 84, y: 28, lat: 34.21, lng: -118.46 },
      { id: 'g9', name: 'San Marino Trees', fruit: 'Lemons', units: 16, x: 86, y: 68, lat: 34.18, lng: -118.43 },
    ],
  },
]
    ...
    >
      ...
      <svg style={{ position: 'absolute', inset: 0, width: '100%', height: '100%' }}>
        {district.growers.map((grower, idx) => {
          const highlighted = highlightedGrowers.length === 0 || highlightedGrowers.includes(grower.name)
          return (
            <line
              key={grower.id}
              x1={`${grower.x}%`}
              ...
              opacity={routesVisible && highlighted ? 1 : 0.08}
              style={{
                transition: `opacity 400ms ease ${idx * 180}ms, stroke 400ms ease, stroke-width 400ms ease`,
              }}
            />
          )
        })}
      ...
      {district.growers.map((grower) => {
        const active = highlightedGrowers.length === 0 || highlightedGrowers.includes(grower.name)
        return (
          <div
            key={grower.id}
            style={{
          ...
          >
            <div
              style={{
                ...
                boxShadow: stage === 'completed' && active
                  ? `0 0 0 10px ${colorForFruit(grower.fruit)}33`
                  : `0 0 0 8px ${colorForFruit(grower.fruit)}22`,
              }}
            />
            <div
              style={{
      ...
      <div
        style={{
          position: 'absolute',
          left: `${district.store.x}%`,
          top: `${district.store.y}%`,
          transform: 'translate(-50%, -50%)',
        }}
      >
        <div
          style={{
            ...
            boxShadow: stage === 'store-execution' || stage === 'completed'
              ? '0 0 0 14px rgba(16,32,51,0.18)'
              : '0 0 0 8px rgba(16,32,51,0.10)',
          }}
        />
        <div
          style={{
...
export default function DistrictManagerMode({ listings }: { listings: Listing[] }) {
  ...
  const [highlightedGrowers, setHighlightedGrowers] = useState<string[]>([])
  ...
  async function seedDistrictData() {
    ...
    await fetch('/api/shortage', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        store: district.store.name,
        item: district.item,
        units: district.store.shortageQty,
        lat: district.store.lat,
        lng: district.store.lng,
      }),
    })
  }
  ...
  async function runLiveExecution() {
    ...
    setSummary(`Scanning live district inputs for ${district.store.shortageFruit.toLowerCase()} demand.`)
    ...
    const response = await fetch('/api/execute', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ item: district.item }),
    })

    const data: ExecuteResponse = await response.json()
    const matches = data.matches || []
    const growers = matches.map((m) => m.grower)

    setHighlightedGrowers(growers)
    setCoverage(matches.length ? 82 : 40)
    setMatched(matches.length)
    setSuppliersNotified(matches.length)
    setEta(matches.length ? '12 min' : '—')
    setUnitsSecured(Number(data.fulfilled || 0))
    setSummary(matches.length
      ? `Live execution matched ${matches.length} growers and secured ${data.fulfilled || 0} units.`
      : 'Live execution found no usable growers for this shortage.')
    await new Promise((r) => setTimeout(r, 900))

    setStage('store-execution')
    setCoverage(matches.length ? 96 : 40)
    setEta(matches.length ? '9 min' : '—')
    await new Promise((r) => setTimeout(r, 900))

    setStage('completed')
    setCoverage(matches.length ? 100 : 40)
    setSummary(matches.length
      ? `Live fill executed. ${data.fulfilled || 0} units committed to ${district.store.name}.`
      : 'Execution completed with no live match.')
  }
  ...
  async function runExecution() {
    ...
    try {
      if (backendMode === 'live') {
        ...
        setEta('22 min')
        setHighlightedGrowers(district.growers.map((g) => g.name))
      ...
      console.error(error)
      setStage('idle')
      setSummary('Live execution failed. Check backend and try again.')
    } finally {
      setIsRunning(false)
    }
  }
  ...
  async function runAutoPlay() {
    ...
    for (const id of ['sfv', 'la-core', 'pasadena']) {
      setDistrictId(id)
  ...
  const buttonLabel =
    stage === 'idle'
      ? backendMode === 'live' ? 'Execute Live Fill' : 'Execute Local Fill'
      : stage === 'market-scan'
      ? 'Scanning Market...'
      ...
      <div style={{ maxWidth: 1360, margin: '0 auto' }}>
        ...
        >
          <div>
            ...
            <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap', marginTop: 22 }}>
              ...
              <Pill>{stats.supply} units live supply</Pill>
            </div>

            <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', marginTop: 18 }}>
              <ToggleButton active={role === 'district-manager'} onClick={() => setRole('district-manager')}>
                District Manager
              </ToggleButton>
              <ToggleButton active={role === 'regional-vp'} onClick={() => setRole('regional-vp')}>
                Regional VP
              </ToggleButton>
              <ToggleButton active={role === 'national-retail'} onClick={() => setRole('national-retail')}>
                National Retail
              </ToggleButton>
            </div>

            <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', marginTop: 12 }}>
              <ToggleButton active={backendMode === 'demo'} onClick={() => setBackendMode('demo')}>
                Demo Mode
              </ToggleButton>
              <ToggleButton active={backendMode === 'live'} onClick={() => setBackendMode('live')}>
                Live Mode
              </ToggleButton>
            </div>

            <div style={{ marginTop: 12, color: 'rgba(255,255,255,0.96)', fontWeight: 700, fontSize: 14 }}>
              {roleSubtitle} {backendMode === 'live' ? 'Live mode uses backend execution.' : 'Demo mode uses built-in sample logic.'}
            </div>
          </div>
          ...
          >
            ...
            <div style={{ marginTop: 10, fontSize: 30, fontWeight: 900, lineHeight: 1.04 }}>
              {district.store.name} → {district.store.shortageFruit}
            </div>
        ...
        <section style={{ display: 'grid', gridTemplateColumns: '1.15fr 0.85fr', gap: 22, marginTop: 22 }}>
          ...
          >
            ...
            <MapDemo district={district} stage={stage} highlightedGrowers={highlightedGrowers} />
          </div>
            ...
            >
              ...
              <div style={{ display: 'grid', gap: 12, marginTop: 16 }}>
                {district.growers.map((g) => {
                  const active = highlightedGrowers.length === 0 || highlightedGrowers.includes(g.name)
                  return (
                    <div
                      key={g.id}
                      style={{
                        borderRadius: 18,
                        padding: 16,
                        background: active ? '#f8fbff' : '#fcfcfd',
