...
BACKUP_DIR="$ROOT/.pluck-backups/fix-district-and-home-cta-$(date +%Y%m%d-%H%M%S)"
...
if 'import DistrictManagerMode from ' not in text:
    raise SystemExit('DistrictManagerMode import missing from App.tsx')

if 'path="/district"' not in text:
    text = text.replace(
        '<Route path="*" element={<Navigate to="/" replace />} />',
        '          <Route path="/district" element={<DistrictManagerMode listings={listings} />} />\n          <Route path="*" element={<Navigate to="/" replace />} />'
    )

if "District mode" in text and "/district" not in text:
    pass

home_button_snippet = """<button className="ghost" onClick={() => goTo('/district')}>District demo</button>"""

if "goTo('/district')" not in text:
    target = "<button className=\"ghost\" onClick={() => goTo('/map')}>Explore nearby map</button>"
    if target in text:
        text = text.replace(
            target,
            target + "\n                <button className=\"ghost\" onClick={() => goTo('/district')}>District demo</button>",
            1
        )
    else:
        target2 = "<button className=\"ghost\" onClick={() => navigate('/map')}>Open map</button>"
        if target2 in text:
            text = text.replace(
                target2,
                target2 + "\n                <button className=\"ghost\" onClick={() => navigate('/district')}>District demo</button>",
                1
            )

app.write_text(text)
print('App.tsx updated with /district route and home CTA if target found')
PY

echo "Backups saved to $BACKUP_DIR"
echo "District rebuilt with role toggles, district toggles, Execute Local Fill, Run Executive Demo, and home CTA injection attempt."
